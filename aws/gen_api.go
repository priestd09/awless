...
func (s *Infra) FetchResources() (*graph.Graph, error) {
	...
	var containertaskList []*ecs.TaskDefinition
	...
	if s.config.getBool("aws.infra.containertask.sync", true) {
		...
		go func() {
			...
			var resGraph *graph.Graph
			var err error
			resGraph, containertaskList, err = s.fetch_all_containertask_graph()
			if err != nil {
				errc <- err
				return
			}
		...
		s.log.Verbose("sync: *disabled* for resource infra[containertask]")
	}
...
func (s *Infra) FetchByType(t string) (*graph.Graph, error) {
	...
	case "containercluster":
		...
		return graph, err
	case "containertask":
		graph, _, err := s.fetch_all_containertask_graph()
		return graph, err
	case "container":
